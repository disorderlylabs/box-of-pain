...
void dump(const char *name, struct run *run)
{
	...
	std::ofstream dot(options.outdir + outf + ".dot");

	fprintf(stderr, "Starting output on %s%s.dot\n", options.outdir, outf.c_str());
	...
	for (auto tr : run->thread_list)
	{
		...
		defs += "start" + std::to_string(tr->tid) + " [label=\"" + std::to_string(tr->id) + ":" + std::to_string(tr->tid) + ":" + inv + "\",style=\"filled\",fillcolor=\"#1111aa22\"];\n";
		...
		for (; n; n = n->outgoing.front())
		{
			...
			output += nname + " -> ";
			defs += nname + "[label=\"" + std::to_string(tr->id) + ":" + (n->ev->entry ? "entry" : "exit") + ":" + syscall_table[n->ev->sc->number].name + ":" + std::to_string(n->ev->uuid) + "\",model=\"circuit\",group=\"G" + std::to_string(tr->id) + "\",fillcolor=\"" + (is_faulted ? "#6600ff88" : "#00ff0011") + "\",style=\"filled\"];\n";
		}

		output += "exit" + std::to_string(tr->id) + " [constraint=\"true\"];\n};\n";
		defs += "exit" + std::to_string(tr->id) + " [label=\"Exit: " + std::to_string(tr->proc->ecode) + "\",style=\"filled\",fillcolor=\"" + (tr->proc->ecode == 0 ? "#1111aa22" : "#ff111188") + "\"];\n";
	}
...
void dump_old(const char *name, struct run *run)
{
	...
	for (auto tr : run->thread_list)
	{
		...
		for (auto e : tr->event_seq)
		{
			...
			bool is_faulted = run->fault_node_set.find(std::pair<int, int>(tr->id, e->uuid)) != run->fault_node_set.end();
			if (e->entry)
			{
				fprintf(dotdefs,
						"e%s "
