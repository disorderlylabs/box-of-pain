...
struct thread_tr *wait_for_syscall(void)
{
	...
	while (!keyboardinterrupt)
	{
		...
		if ((((status >> 16) & 0xffff) == PTRACE_EVENT_CLONE) || (((status >> 16) & 0xffff) == PTRACE_EVENT_FORK) || (((status >> 16) & 0xffff) == PTRACE_EVENT_VFORK))
		{
...
int do_trace()
{
	...
	for (auto tr : current_run.thread_list)
	{
		...
		if (ptrace(PTRACE_SETOPTIONS,
				   tr->tid,
				   0,
				   PTRACE_O_TRACESYSGOOD | PTRACE_O_TRACECLONE | PTRACE_O_EXITKILL | PTRACE_O_TRACEFORK | PTRACE_O_TRACEVFORK) != 0)
		{
